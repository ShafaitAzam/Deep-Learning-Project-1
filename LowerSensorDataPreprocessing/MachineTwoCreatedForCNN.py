...
spectrograms_list = []
for j in range(0,5249):
    sp = np.load(f'C:/ML/RawDataCollectedFromSensors/LowerSensorDataPreprocessing/NumpySpectrograms/spectrogram_{j}.npy')
...
encoder.fit(yl)
encoded_Y = encoder.transform(yl)
...
#Training of the CNN
history = model.fit(train_spectrograms2, train_labels, batch_size=64, epochs=10, validation_split=0.2)
